...
components/                 화면 조립용 UI 블록. 화면(app/)에서 콘텐츠를 받아 그리기만 한다
  ...
  map/DeviceMap.tsx           SVG 개략도 지도 + 핀 halo 펄스 + 주소 오버레이
...
| # | 질문 | 지금 임시 처리 |
|---|---|---|
| [O1](../../planning/decisions/open-questions.md#o1) | 위치를 어떻게 잡나 (GPS/등록위치/게이트웨이) | `DeviceMap`은 고정 주소 문자열만 표시. 실제 지도 라이브러리 미연동 |
...
npm run web          # 웹으로 빠르게 확인 (지도·차트가 react-native-svg라 웹에서도 그대로 보인다)
...
`react-native-svg`(지도·차트)와 `expo-linear-gradient`(위험도 바·경보 명멸)를 추가했다. 둘 다 `npx expo install`로 넣은 SDK 54 호환 버전이라 임의로 다른 버전으로 올리지 말 것.
