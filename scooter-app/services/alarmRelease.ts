...
//
// 실제 엔드포인트: POST /v1/devices/{mac}/alarm/release, body { note?: string },
// 응답 { released: boolean } (types/telemetry.ts의 AlarmReleaseRequest/Response 참고).
// 아직 이 파일은 실제 HTTP 호출 없이 항상 실패하는 스텁만 있다 — deviceRegistry.ts의
// localOnlyDeviceRegistry(항상 성공)와 다르게, 이건 진짜 서버 쪽 권한 로직이 있어야
// 의미가 있는 액션이라 "아직 안 붙였음"을 정직하게 실패로 표시한다.
