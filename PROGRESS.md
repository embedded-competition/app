...
| 항목 상세 | ✅ 완료(2026-08-24부터 판정·게이지·최근 1분 차트는 실 API 연동) | 채널별로 실측값 반영(`conditions[]` 기반 색·문구, `useChannelHistory`가 쌓은 실시간 차트). 판단근거·비교는 여전히 목데이터(대응 API 없음), temp·leak 채널도 목데이터 |
