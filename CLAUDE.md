...
([참고](https://cobinding.tistory.com/165) 요약 — 명세 없으면 프론트/백엔드가 각자 다르게 짜서(`/api/resources` vs `/api/resource` 같은 불일치) 시간 낭비한다는 게 핵심 문제의식.)

- **문서는 개요 → 사용 순서 → API 레퍼런스** 3단 구성을 따른다. 개요에 서비스 목적, 사용 순서에 "어떤 API를 어떤 순서로 부르는지"(예: 기기 등록 → 조회 반복 → 경보 시 해제 요청), 레퍼런스에 API별 요청/응답을 둔다.
- **성공/실패 1차 판단은 HTTP 상태 코드로 한다** (200번대 성공 / 400번대 클라이언트 오류 / 500번대 서버 오류). 바디 안의 `ok` 필드는 보조 정보지 상태 코드를 대체하지 않는다.
- **공통 에러 코드는 한 곳에 모아두고 각 API에서는 거기로 링크**한다 — API마다 에러 목록을 반복해서 쓰지 않는다.
- 요청/응답은 항상 **실제 JSON 예시**로 보여준다(표로 필드만 나열하지 않는다) — `scooter-app/docs/interface.md`가 정의한 필드명·타입과 반드시 맞출 것.
...
이 레포(`app`)에는 실제로 `scooter-app/` 앱 코드 하나만 있다. **아래 폴더 구조·명령어·경로는 전부 `scooter-app/` 기준이다** — 명령어는 그 디렉토리에서 실행할 것.
...
아래는 모두 `scooter-app/` 디렉토리에서 실행한다.
